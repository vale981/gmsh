...
class model:
    ...
    class mesh:
        ...
        @staticmethod
        def setSizeCallback(sizeCallback):
            """
            ...
            - `sizeCallback': 
            """
            global api_sizeCallback_type_
            api_sizeCallback_type_ = CFUNCTYPE(c_double, c_int, c_int, c_double, c_double, c_double, c_double, c_void_p)
            global api_sizeCallback_
            api_sizeCallback_ = api_sizeCallback_type_(lambda dim, tag, x, y, z, lc, _ : sizeCallback(dim, tag, x, y, z, lc))
            ierr = c_int()
            lib.gmshModelMeshSetSizeCallback(
                api_sizeCallback_, None,
                byref(ierr))
            if ierr.value != 0:
                raise Exception(logger.getLastError())
        ...
        @staticmethod
        def setMetricCallback(metricCallback):
            """
            gmsh.model.mesh.setMetricCallback(metricCallback)

            Set a mesh size callback for the current model. The callback function
            should take seven arguments as input (`dim', `tag', `x', `y', `z', `lc' and
            `metric'). The first two integer arguments correspond to the dimension
            `dim' and tag `tag' of the entity being meshed. The next four double
            precision arguments correspond to the coordinates `x', `y' and `z' around
            which to prescribe the mesh size and to the mesh size `lc' that would be
            prescribed if the callback had not been called. The seventh argument
            `metric' is a vector of double precision numbers (std::array in C++)
            containing the components of the symmetric metric tensor as it would be
            prescribed if the callback had not been called. The callback function can
            modify the `metric' in place or return a new vector of double precision
            numbers specifying the desired mesh size(s): a single number specifies an
            isotropic size and 6 numbers specify a symmetric anisotropic metric (the 6
            components of the lower triangular part of the 3x3 matrix, in the order
            M11, M21, M22, M31, M32, M33). For convenience, in some languages (Python,
            Julia), a full 3x3 matrix can also be returned: the 6 unique components
            will then be extracted to define the symmetric metric.

            Types:
            - `metricCallback': 
            """
            global api_metricCallback_type_
            api_metricCallback_type_ = CFUNCTYPE(None, c_int, c_int, c_double, c_double, c_double, c_double, POINTER(c_double), c_void_p)
            def metricCallback_wrapper(dim, tag, x, y, z, lc, metric, data):
                try:
                    import numpy as np
                ...
                if isinstance(res, (float, int)): 
                    ...
                    try:
                        ...
                        for i in range(6): metric[i] = 0.
                        metric[0] = metric[2] = metric[5] = 1. / (res * res)
            global api_metricCallback_
            api_metricCallback_ = api_metricCallback_type_(metricCallback_wrapper)
            ierr = c_int()
            lib.gmshModelMeshSetMetricCallback(
                api_metricCallback_, None,
                byref(ierr))
            if ierr.value != 0:
                raise Exception(logger.getLastError())
        set_metric_callback = setMetricCallback

        @staticmethod
        def removeMetricCallback():
            """
            ...
            """
            ierr = c_int()
            lib.gmshModelMeshRemoveMetricCallback(
                byref(ierr))
            if ierr.value != 0:
                raise Exception(logger.getLastError())
        remove_metric_callback = removeMetricCallback
