...
"""
    gmsh.model.mesh.setSizeCallback(sizeCallback)
...
Types:
 - `sizeCallback`: 
"""
function setSizeCallback(sizeCallback)
    api_sizeCallback__(dim, tag, x, y, z, lc, data) = sizeCallback(dim, tag, x, y, z, lc)
    api_sizeCallback_ = @cfunction($api_sizeCallback__, Cdouble, (Cint, Cint, Cdouble, Cdouble, Cdouble, Cdouble, Ptr{Cvoid}))
    ierr = Ref{Cint}()
    ccall((:gmshModelMeshSetSizeCallback, gmsh.lib), Cvoid,
          (Ptr{Cvoid}, Ptr{Cvoid}, Ptr{Cint}),
          api_sizeCallback_, C_NULL, ierr)
    ierr[] != 0 && error(gmsh.logger.getLastError())
    return nothing
end
...
"""
    gmsh.model.mesh.setMetricCallback(metricCallback)

Set a mesh size callback for the current model. The callback function should
take seven arguments as input (`dim`, `tag`, `x`, `y`, `z`, `lc` and `metric`).
The first two integer arguments correspond to the dimension `dim` and tag `tag`
of the entity being meshed. The next four double precision arguments correspond
to the coordinates `x`, `y` and `z` around which to prescribe the mesh size and
to the mesh size `lc` that would be prescribed if the callback had not been
called. The seventh argument `metric` is a vector of double precision numbers
(std::array in C++) containing the components of the symmetric metric tensor as
it would be prescribed if the callback had not been called. The callback
function can modify the `metric` in place or return a new vector of double
precision numbers specifying the desired mesh size(s): a single number specifies
an isotropic size and 6 numbers specify a symmetric anisotropic metric (the 6
components of the lower triangular part of the 3x3 matrix, in the order M11,
M21, M22, M31, M32, M33). For convenience, in some languages (Python, Julia), a
full 3x3 matrix can also be returned: the 6 unique components will then be
extracted to define the symmetric metric.

Types:
 - `metricCallback`: 
"""
function setMetricCallback(metricCallback)
    function api_metricCallback__(dim::Cint, tag::Cint, x::Cdouble, y::Cdouble, z::Cdouble, lc::Cdouble, metric::Ptr{Cdouble}, data::Ptr{Cvoid})
        c_m = unsafe_wrap(Array, metric, 6)
        res = metricCallback(dim, tag, x, y, z, lc, c_m)
        if res === nothing
    ...
    api_metricCallback_ = @cfunction($api_metricCallback__, Cvoid, (Cint, Cint, Cdouble, Cdouble, Cdouble, Cdouble, Ptr{Cdouble}, Ptr{Cvoid}))
    ierr = Ref{Cint}()
    ccall((:gmshModelMeshSetMetricCallback, gmsh.lib), Cvoid,
          (Ptr{Cvoid}, Ptr{Cvoid}, Ptr{Cint}),
          api_metricCallback_, C_NULL, ierr)
    ierr[] != 0 && error(gmsh.logger.getLastError())
    return nothing
end
const set_metric_callback = setMetricCallback

"""
    gmsh.model.mesh.removeMetricCallback()

Remove the mesh metric callback from the current model.
"""
function removeMetricCallback()
    ierr = Ref{Cint}()
    ccall((:gmshModelMeshRemoveMetricCallback, gmsh.lib), Cvoid,
          (Ptr{Cint},),
          ierr)
    ierr[] != 0 && error(gmsh.logger.getLastError())
    return nothing
end
const remove_metric_callback = removeMetricCallback
