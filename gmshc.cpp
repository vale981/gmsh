...
GMSH_API void gmshModelMeshSetSizeCallback(double (*sizeCallback)(int dim, int tag, double x, double y, double z, double lc, void * data), void * sizeCallback_data, int * ierr)
{
  if(ierr) *ierr = 0;
  try {
    gmsh::model::mesh::setSizeCallback(std::bind(sizeCallback, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5, std::placeholders::_6, sizeCallback_data));
  }
  catch(...){
    if(ierr) *ierr = 1;
  }
...
GMSH_API void gmshModelMeshSetMetricCallback(void (*metricCallback)(int dim, int tag, double x, double y, double z, double lc, double * metric, void * data), void * metricCallback_data, int * ierr)
{
  if(ierr) *ierr = 0;
  try {
struct metricCallback_caller_  {
    ...
    gmsh::model::mesh::setMetricCallback(std::bind(&metricCallback_caller_::call, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5, std::placeholders::_6, std::placeholders::_7, metricCallback_data, metricCallback));
  }
  catch(...){
    if(ierr) *ierr = 1;
  }
...
GMSH_API void gmshModelMeshRemoveMetricCallback(int * ierr)
{
  if(ierr) *ierr = 0;
  try {
    gmsh::model::mesh::removeMetricCallback();
  }
  catch(...){
    if(ierr) *ierr = 1;
  }
